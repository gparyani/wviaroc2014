 ...
 */
public class Rac3TruckMovement_BackoffWithTacho {
	...
	public static void goBackwards(int power, int bearing, int ticks)
	{
		left.stop();
		right.stop();
		goBackwards(power,bearing);
		int currentTicks = getRightTachoCount();
		while(Math.abs(currentTicks-getRightTachoCount())<ticks&&!right.isStalled()) {		}
		
	}
	
	public static void goForwards(int power, int bearing, int ticks)
	{
		left.stop();
		right.stop();
